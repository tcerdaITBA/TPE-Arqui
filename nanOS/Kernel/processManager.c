...
static uint64_t fill_stack(uint64_t rip, uint64_t rsp);
...
void exec_process(uint64_t new_process_rip) {
	process new_process;
	new_process.stack_page = get_stack_page(); /* Pide al MemoryAllocator espacio para el stack */

	new_process.rsp = fill_stack(new_process_rip, new_process.stack_page);
...
** https://bitbucket.org/RowDaBoat/wyrm */
static uint64_t fill_stack(uint64_t rip, uint64_t stack_page) {
